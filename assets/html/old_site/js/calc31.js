...
CVSS.vectorStringRegex_31 = /^CVSS:3\.1\/((AV:[NALP]|AC:[LH]|PR:[UNLH]|UI:[NR]|S:[UC]|[CIA]:[NLH]|E:[XUPFH]|RL:[XOTWU]|RC:[XURC]|[CIA]R:[XLMH]|MAV:[XNALP]|MAC:[XLH]|MPR:[XUNLH]|MUI:[XNR]|MS:[XUC]|M[CIA]:[XNLH])\/)*(AV:[NALP]|AC:[LH]|PR:[UNLH]|UI:[NR]|S:[UC]|[CIA]:[NLH]|E:[XUPFH]|RL:[XOTWU]|RC:[XURC]|[CIA]R:[XLMH]|MAV:[XNALP]|MAC:[XLH]|MPR:[XUNLH]|MUI:[XNR]|MS:[XUC]|M[CIA]:[XNLH])$/;
CVSS.Weight = {
    AV: {
        N: 0.85,
        A: 0.62,
...
CVSS.calculateCVSSFromMetrics = function(AttackVector, AttackComplexity, PrivilegesRequired, UserInteraction, Scope, Confidentiality, Integrity, Availability, ExploitCodeMaturity, RemediationLevel, ReportConfidence, ConfidentialityRequirement, IntegrityRequirement, AvailabilityRequirement, ModifiedAttackVector, ModifiedAttackComplexity, ModifiedPrivilegesRequired, ModifiedUserInteraction, ModifiedScope, ModifiedConfidentiality, ModifiedIntegrity, ModifiedAvailability) {
    ...
    if (impact <= 0) {
        ...
        if (S === 'U') {
            baseScore = CVSS.roundUp1(Math.min((exploitability + impact), 10))
        } else {
            baseScore = CVSS.roundUp1(Math.min(CVSS.scopeCoefficient * (exploitability + impact), 10))
        }
    }
    ...
    if (modifiedImpact <= 0) {
        ...
        envScore = CVSS.roundUp1(CVSS.roundUp1(Math.min((modifiedImpact + modifiedExploitability), 10)) * metricWeightE * metricWeightRL * metricWeightRC)
    } else {
        envScore = CVSS.roundUp1(CVSS.roundUp1(Math.min(CVSS.scopeCoefficient * (modifiedImpact + modifiedExploitability), 10)) * metricWeightE * metricWeightRL * metricWeightRC)
    }
...
CVSS.calculateCVSSFromVector = function(vectorString) {
    ...
    var badMetrics = [];
    if (!CVSS.vectorStringRegex_31.test(vectorString)) {
        return {
            success: !1,
...
;
CVSS.roundUp1 = function Roundup(input) {
